...
template class TexturePipeline<XMFLOAT4>;
...
void TexturePipeline<T>::CreatePipeline()
{
    ...
    std::vector<CD3DX12_ROOT_PARAMETER> rootParameters = std::vector<CD3DX12_ROOT_PARAMETER>(m_constantBuffers.size() + 1);
...
void TexturePipeline<T>::Execute(const UINT threadX, const UINT threadY, const UINT threadZ)
{
    ...
    m_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_rsc.Get(), D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_UNORDERED_ACCESS));

    m_commandList->Dispatch(threadX, threadY, threadZ);

    m_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::UAV(m_rsc.Get()));

    m_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_rsc.Get(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE));
