...
template <typename T>
void ComputePipeline<T>::Execute(const std::vector<T*>& data, const UINT threadX, const UINT threadY,
                                 const UINT threadZ)
{
	std::vector<T> values = {};
	for (T* item : data)
	...
	for (int i = 0; i < data.size(); i++)
		memcpy(data[i], static_cast<T*>(m_data) + i, sizeof(T));
}
