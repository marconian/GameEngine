...
void Text::Print(std::string text, Vector2 position, Align align, DirectX::XMVECTORF32 color)
{
    ID3D12GraphicsCommandList* commandList = g_deviceResources->GetCommandList();
...
void Text::CreateDeviceDependentResources()
{
    ...
    auto uploadResourcesFinished = resourceUpload.End(g_deviceResources->GetCommandQueue());
