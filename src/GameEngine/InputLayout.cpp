...
D3D12_INPUT_LAYOUT_DESC InputLayout::GetLayout()
{
	...
	inputLayout.NumElements = static_cast<UINT>(elements->size());
	inputLayout.pInputElementDescs = elements->data();

	return inputLayout;
}

D3D12_INPUT_ELEMENT_DESC InputLayout::GetInputElementDesc(const InputElement& element, const unsigned int offset)
{
	DXGI_FORMAT format;
	...
	else if (element.type == typeid(DirectX::XMFLOAT2).name() || element.type == typeid(DirectX::SimpleMath::Vector2).
		name())
	{
		format = DXGI_FORMAT_R32G32_FLOAT;
	}
	else if (element.type == typeid(DirectX::XMFLOAT3).name() || element.type == typeid(DirectX::SimpleMath::Vector3).
		name())
	{
		format = DXGI_FORMAT_R32G32B32_FLOAT;
	}
	else if (element.type == typeid(DirectX::XMFLOAT4).name() || element.type == typeid(DirectX::SimpleMath::Vector4).
		name() || element.type == typeid(DirectX::XMVECTORF32).name())
	{
